...
# ~\~ begin <<docs/04-implementation.md|parareal/futures.py>>[init]
...
# ~\~ begin <<docs/04-implementation.md|parareal-futures>>[init]
def combine(c1: Vector, f1: Vector, c2: Vector) -> Vector:
    return c1 + f1 - c2
# ~\~ end
# ~\~ begin <<docs/04-implementation.md|parareal-futures>>[1]
@dataclass
class Parareal:
    client: Client
    ...
    # ~\~ begin <<docs/04-implementation.md|parareal-methods>>[init]
    def step(self, n_iter: int, y_prev: list[Future], t: NDArray[np.float64]) -> list[Future]:
        ...
        return y_next
    # ~\~ end
    # ~\~ begin <<docs/04-implementation.md|parareal-methods>>[1]
    def schedule(self, y_0: Vector, t: NDArray[np.float64]) -> list[list[Future]]:
        ...
        return jobs
    # ~\~ end
    # ~\~ begin <<docs/04-implementation.md|parareal-methods>>[2]
